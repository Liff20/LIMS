...
@section('content')
    ...
    <div class="card p-5">
        <div class="mb-4 flex items-center justify-between">
            ...
            <button class="btn-primary" type="button" x-data x-on:click="$dispatch('open-modal', 'modal-jenis-barang')">
                            ...
                            <td class="text-right">
                                <div class="inline-flex gap-1">
                                    <button class="btn-ghost !px-2.5 !py-1.5" title="Edit"
                                        x-data x-on:click="$dispatch('open-modal', 'modal-jenis-barang-{{ $j->id }}')">
                                        <x-icon name="edit" class="h-4 w-4" />
                                    </button>
                                    <form method="POST" action="{{ route('konfigurasi.jenis-barang.destroy', $j->id) }}"
    ...
    <x-modal id="modal-jenis-barang" title="Tambah Jenis Barang">
        <form method="POST" action="{{ route('konfigurasi.jenis-barang.store') }}">
            @csrf
            <div class="space-y-3">
                <div>
    ...
    @foreach ($items as $j)
        <x-modal id="modal-jenis-barang-{{ $j->id }}" title="Edit Jenis Barang">
            <form method="POST" action="{{ route('konfigurasi.jenis-barang.update', $j->id) }}">
                @csrf
                @method('PUT')
                <div class="space-y-3">
                    <div>
